...
class NativeOllamaChain:
    ...
    def invoke(self, input_dict: dict) -> AuditFindingSchema:
        ...
        try:
            ...
            # FIX 5: Return a clearly flagged PARSE_ERROR finding instead of a silent
            # generic NON_COMPLIANT, so failures are visible in the report and don't
            # corrupt accuracy metrics by masquerading as legitimate audit results.
            ...
            return AuditFindingSchema(
                ...
                control_coverage=0,
                evidence_count=0,
                business_impact=f"[PARSE_ERROR] Auditor engine could not parse model output for control {control_id_str}. Manual verification required.",
                remediation_priority="High" if control_default_severity in ("High", "Critical") else "Medium",
                justification=f"[PARSE_ERROR] The AI model returned an unparseable response for control {control_id_str}. "
                              f"This finding was generated by the error fallback and does NOT reflect actual document analysis. "
                              f"Technical error: {str(e)}",
                missing_requirements=[f"[PARSE_ERROR] Manual verification required for {control_id_str} — model output was unparseable."],
                recommendation=f"[PARSE_ERROR] Requires manual review. Unable to automatically assess control {control_id_str} due to model output parsing failure.",
                evidence=[]
            )
