...
function onFrameworkChangeSuggestMode() {
    ...
    const isVapt = frameworkSelect ? frameworkSelect.value.toUpperCase().includes("VAPT") : false;
    ...
    if (isVapt) {
        ...
        // Switching away from VAPT back to ISO (or any other framework) -- VAPT
        // Scan mode is meaningless outside VAPT, and leaving it selected caused
        // an ISO-framework audit to keep running VAPT-style scanner-file checks.
        // Only reset if VAPT Scan is what's currently active; if the user is on
        // Quick/Deep already, leave their choice alone.
...
async function loadFrameworkControls() {
    ...
    try {
        ...
        const filtered = controlsToRender.filter(c => {
            ...
            const isIso = !isVapt && !isDpdp && !isSoc2 && !isBcms && !isXbom && !isNist;
