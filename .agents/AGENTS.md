...
`FINAL_RESULT = COMPLIANT` only if ALL of: `POLICY_STATUS=FOUND` AND `POLICY_ASSESSMENT=COMPLIANT`
AND `EVIDENCE_STATUS=FOUND` AND `EVIDENCE_ASSESSMENT=COMPLIANT` AND `POLICY_VALIDITY` is acceptable
AND `EVIDENCE_FRESHNESS` is acceptable. Otherwise `NON_COMPLIANT`. The top-level status should equal
`FINAL_RESULT` unless the applicability check set it to `FALSE_POSITIVE`.
