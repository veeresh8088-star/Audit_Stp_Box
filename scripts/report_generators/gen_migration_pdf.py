...
class MigrationReportPDF(FPDF):
    def header(self):
        self.set_font('Helvetica', 'B', 9)
        self.set_text_color(100, 116, 139)
        self.cell(0, 8, 'AICyberAuditBox - Migration & Token Architecture Report', align='R', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
...
def clean(val):
    ...
    val = val.replace('🟢', '[PASS]').replace('🔴', '[GAP]').replace('⚠️', '[NOTE]').replace('💾', '[SAVE]').replace('⛔', '[STOP]')
    val = val.encode('latin-1', 'replace').decode('latin-1')
    return val

def generate_pdf():
    pdf = MigrationReportPDF()
    pdf.set_auto_page_break(auto=True, margin=15)
    pdf.set_margins(15, 15, 15)
    pdf.add_page()

    md_file = 'MIGRATION_AND_TOKEN_ARCHITECTURE_REPORT.md'
    ...
    output_pdf = 'MIGRATION_AND_TOKEN_ARCHITECTURE_REPORT.pdf'
    pdf.output(output_pdf)
    print(f"[SUCCESS] Report PDF generated successfully: {output_pdf}")
