...
class PackagingReportPDF(FPDF):
    def header(self):
        self.set_font('Helvetica', 'B', 9)
        self.set_text_color(100, 116, 139)
        self.cell(0, 8, 'AICyberAuditBox - Desktop Packaging & Cross-Platform Deployment Guide (.exe & .app)', align='R', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
...
def clean(val):
    ...
    val = val.replace('📦', '[PKG]').replace('🛠️', '[BUILD]').replace('⚡', '[FAST]').replace('🏆', '[BEST]').replace('💡', '[NOTE]')
    val = val.encode('latin-1', 'replace').decode('latin-1')
    return val

def generate_pdf():
    pdf = PackagingReportPDF()
    pdf.set_auto_page_break(auto=True, margin=15)
    pdf.set_margins(15, 15, 15)
    pdf.add_page()

    md_file = 'DESKTOP_PACKAGING_AND_DEPLOYMENT_GUIDE.md'
    ...
    output_pdf = 'DESKTOP_PACKAGING_AND_DEPLOYMENT_GUIDE.pdf'
    pdf.output(output_pdf)
    print(f"[SUCCESS] Packaging Guide PDF generated successfully: {output_pdf}")
