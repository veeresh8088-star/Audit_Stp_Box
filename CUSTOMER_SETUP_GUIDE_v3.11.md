# AICyberAuditBox v3.11 — Customer Setup & Operations Guide

## Overview
This document provides simple instructions for installing, operating, and applying routine software updates for **AICyberAuditBox v3.11** on your local PC or server. The system operates **100% offline (air-gapped)** with zero internet dependency.
...
* **Type B: Routine Delta Update Package (App Code / Bug Fix Updates)**
  - `aicyberauditbox_delta_app_v3.11.tar` (~50–100 MB small update file)
  - `aicyberauditbox_delta_app_v3.11_companion.zip` (~5 KB configuration update)
...
When you receive a **Routine Delta Update Package** (e.g., `v3.11` code fixes or prompt enhancements), follow these **2 fast steps**:

```bash
# Step 1: Load ONLY the updated app image (Takes ~5 seconds)
docker load -i aicyberauditbox_delta_app_v3.11.tar

# Step 2: Replace compose file & restart ONLY the app container
unzip -o aicyberauditbox_delta_app_v3.11_companion.zip
