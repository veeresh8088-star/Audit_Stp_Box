...
class BestChoicePDF(FPDF):
    def header(self):
        self.set_font('Helvetica', 'B', 9)
        self.set_text_color(100, 116, 139)
        self.cell(0, 8, 'AICyberAuditBox - Executive Choice & Packaging Architecture', align='R', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
...
def clean(val):
    ...
    val = val.replace('🏆', '[TOP]').replace('📦', '[PKG]').replace('⚡', '[FAST]').replace('🛠️', '[BUILD]').replace('💻', '[TECH]')
...
def generate_pdf():
    ...
    md_file = 'AICyberAuditBox_Best_Choice_Packaging_Report.md'
    ...
    output_pdf = 'AICyberAuditBox_Best_Choice_Packaging_Report.pdf'
    pdf.output(output_pdf)
    print(f"[SUCCESS] Best Choice PDF generated successfully: {output_pdf}")
